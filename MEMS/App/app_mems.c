...
static void Gyro_Sensor_Handler(uint32_t Instance)
{
  ...
  else
  {
   // snprintf(dataOut, MAX_BUF_SIZE, "\r\nGYR_X[%d]: %d, GYR_Y[%d]: %d, GYR_Z[%d]: %d\r\n", (int)Instance,
    	    snprintf(dataOut, MAX_BUF_SIZE, "X: %d,Y: %d",
    	             (int)angular_velocity.x, (int)angular_velocity.y);

    	    snprintf(dataOut1, MAX_BUF_SIZE, "Z: %d",
    	        	             (int)angular_velocity.z);
    //lcd_print(1,1,dataOut);
    //lcd_print(2,1,dataOut1);
//test111111111111
  }
...
static void Magneto_Sensor_Handler(uint32_t Instance)
{
  ...
  if (verbose == 1)
  {
    ...
    {
      snprintf(dataOut, MAX_BUF_SIZE, "Magn[%d]: %d gauss", (int)Instance, (int)fullScale);
...
static void Temp_Sensor_Handler(uint32_t Instance)
{
  ...
  else
  {
    ...
    snprintf(dataOut, MAX_BUF_SIZE, "%c%d.%02d *C", ((out_value.sign) ? '-' : ' '), (int)out_value.out_int,
             (int)out_value.out_dec);
...
static void Hum_Sensor_Handler(uint32_t Instance)
{
  ...
  else
  {
    ...
    snprintf(dataOut, MAX_BUF_SIZE, "Wilg[%d]: %d.%02d %%", (int)Instance, (int)out_value.out_int,
             (int)out_value.out_dec);
  //  lcd_print(2,1,dataOut);
  }
